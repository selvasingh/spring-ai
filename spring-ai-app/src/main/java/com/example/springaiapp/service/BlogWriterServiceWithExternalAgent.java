 ...
 */
@Service
public class BlogWriterServiceWithExternalAgent {
    private static final Logger logger = LoggerFactory.getLogger(BlogWriterServiceWithExternalAgent.class);
    private static final int MAX_ITERATIONS = 5;  // Maximum number of refinement iterations
     ...
     */
//    public BlogWriterServiceWithExternalAgent(ChatClient.Builder chatClientBuilder,
//                                              ToolCallbackProvider tools) {
    public BlogWriterServiceWithExternalAgent(ChatClient.Builder chatClientBuilder,
                                              McpSyncClient mcpSyncClient) {
        // Add SimpleLoggerAdvisor to log requests and responses for debugging
        ToolCallbackProvider tools = new SyncMcpToolCallbackProvider(mcpSyncClient);

        List<ToolCallback> syncCallbacks = McpToolUtils.getToolCallbacksFromSyncClients(mcpSyncClient);

        logger.info("Creating BlogWriterServiceWithExternalAgent with ChatClient, SimpleLoggerAdvisor and tools");

        this.chatClient = chatClientBuilder
                .defaultAdvisors(new SimpleLoggerAdvisor())
                .defaultTools(syncCallbacks)
                .build();

        logger.info("Initialized BlogWriterServiceWithExternalAgent with ChatClient {}, SimpleLoggerAdvisor and tools {}",
                this.chatClient, tools);

//        logger.info("tools.getToolCallbacks().length() is {} & tools.getToolCallbacks().toString() is {}",
//                Arrays.asList(tools.getToolCallbacks()).size(),
//                tools.getToolCallbacks());

        logger.info("BlogWriterServiceWithExternalAgent initialized with ChatClient and SimpleLoggerAdvisor");
    }
    ...
    public BlogGenerationResult generateBlogPostWithMetadata(String topic) {
        ...
        while ((!approved && iteration <= MAX_ITERATIONS) || forceFirstIteration) {
            ...
            String evalPrompt = String.format("""
                ...
                - Avoid AI-generated text patterns or clichés
                ...
                6. Must not trigger AI Scanners - use the configure "scan" tool; and share scan outcome as JSON.
            ...
            if (evaluation.toUpperCase().contains("PASS") && iteration > 1) { // Only allow PASS after first iteration
                ...
                String refinePrompt = String.format("""
                    ...
                    9. Ensure the blog does not trigger AI scanners.
                    """, feedback, draft);
